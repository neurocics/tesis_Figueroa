model {
  ...
  mu.beta0 ~ dnorm(0,0.001)      # Intercepto
  sigma.beta0 ~ dunif(0.001,10)
  
  mu.beta1 ~ dnorm(0,0.001)       # EU
  sigma.beta1 ~ dunif(0.001,10)  
  
  # learning rate feedback + 
  alpha_a.kappa    ~ dunif(0.01, 10)
  alpha_a.mu ~ dbeta(1,1)
  alpha_a.a   <- alpha_a.mu *alpha_a.kappa 
  alpha_a.b   <- (1-alpha_a.mu) *alpha_a.kappa 
  # learning rate feedback - 
  alpha_r.kappa    ~ dunif(0.01, 10)
